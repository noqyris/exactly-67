...
export class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    const blockW = Math.min(w * 0.42, u(190))
    ...
    this.add
      .text(w / 2, by + blockH + u(28), 'Balance the scale. Land on exactly 67.', TEXT.ink(16, '600'))
      .setOrigin(0.5)
      .setColor(INK_SOFT)
    ...
    this.scaleView = new ScaleView(this, {
      cx: w / 2,
      cy: by + blockH + h * 0.17,
      halfBeam: Math.min(w * 0.26, u(150)),
      ropeLen: h * 0.075,
      panWidth: Math.min(w * 0.2, u(110)),
      panHeight: Math.min(w * 0.2, u(110)) * 0.22,
    })
    ...
    const levels = makeButton(this, 'Level map', Math.min(w * 0.6, u(260)), u(52), PAPER, '#2B2440', () =>
      this.scene.start('LevelMap', {}),
    )
    levels.setPosition(w / 2, playY + u(74))
    ...
    sound.setPosition(w / 2 - size * 0.75, playY + u(74) + u(64))
    ...
    haptics.setPosition(w / 2 + size * 0.75, playY + u(74) + u(64))
